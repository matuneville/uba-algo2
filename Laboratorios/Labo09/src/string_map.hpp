...
void string_map<T>::copiaRecursivo(Nodo* &n, const Nodo* copy){
    if (copy != nullptr) {
        ...
        if (copy->definicion != nullptr){
            n->definicion = new T(*(copy->definicion));
        }
        for (int i = 0; i < 256; ++i) {
...
void string_map<T>::insert(const pair<string, T>& claveValor){
    ...
    delete actual->definicion; // por si ya estaba definido
...
void string_map<T>::erase(const string& clave) {
    ...
    for (char letra : clave){
        Nodo** pBusco = & buscoNodoRef(actual->siguientes, letra);
...
template<typename T>
typename string_map<T>::Nodo*& string_map<T>::buscoNodoRef(vector<Nodo*> &v, const char &c){
    for(int i = 0; i < 256; i++){
        if (char(i) == c)
            return v[i];
    }
